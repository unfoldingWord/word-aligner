...
var getOrderedVerseObjects = exports.getOrderedVerseObjects = function getOrderedVerseObjects(verseObjects) {
  ...
  var length = _verseObjects.length;
  for (var i = 0; i < length; i++) {
    var verseObject = _verseObjects[i];
  ...
  return { newVerseObjects: _verseObjects, wordMap: wordMap };
};
 ...
 */
var getVerseObjectsText = function getVerseObjectsText(verseObjects) {
  var texts = [];
  if (verseObjects) {
    var length = verseObjects.length;
    for (var i = 0; i < length; i++) {
      var vo = verseObjects[i];
      if (vo.text) {
        texts.push(vo.text);
      }
      if (vo.children) {
        var childTexts = getVerseObjectsText(vo.children);
...
var tokenizeText = function tokenizeText(text, newVerseObjects, wordMap, nonWordVerseObjectCount, verseText) {
  if (text) {
    var tokens = _stringPunctuationTokenizer2.default.tokenizeWithPunctuation(text);
    var tokenLength = tokens.length;
    for (var j = 0; j < tokenLength; j++) {
      var word = tokens[j];
      var verseObject = void 0;
      if (_stringPunctuationTokenizer2.default.word.test(word)) {
        // if the text has word characters, its a word object
        var wordIndex = wordMap.length;
        var occurrence = _stringPunctuationTokenizer2.default.occurrenceInString(verseText, wordIndex, word);
        var occurrences = _stringPunctuationTokenizer2.default.occurrencesInString(verseText, word);
...
var getWordsFromNestedVerseObjects = function getWordsFromNestedVerseObjects(verseObjects, newVerseObjects, wordMap, verseText, nonWordVerseObjectCount) {
  ...
  for (var i = 0; i < voLength; i++) {
    ...
    var vsObjText = verseObject.text && verseObject.text.trim();
...
var getOrderedVerseObjectsFromString = exports.getOrderedVerseObjectsFromString = function getOrderedVerseObjectsFromString(string) {
  ...
  if (string) {
    ...
    var _verseObjectsWithTextString = getVerseObjectsText(_verseObjects).join(' ');

    getWordsFromNestedVerseObjects(_verseObjects, newVerseObjects, wordMap, _verseObjectsWithTextString, 0);
  }
  return { newVerseObjects: newVerseObjects, wordMap: wordMap };
};
 ...
 */
var indexOfVerseObject = exports.indexOfVerseObject = function indexOfVerseObject(wordMap, verseObject) {
  return wordMap.findIndex(function (wordItem) {
    var _verseObject = wordItem.array[wordItem.pos];
...
var getWordListFromVerseObjectArray = exports.getWordListFromVerseObjectArray = function getWordListFromVerseObjectArray(verseObjects) {
  ...
  try {
    for (var _iterator4 = (0, _getIterator3.default)(verseObjects), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
      ...
      wordList.push.apply(wordList, words); // fast arrays
    }
  } catch (err) {
...
var getWordList = exports.getWordList = function getWordList(verseObjects) {
  ...
  if (typeof verseObjects === 'string') {
    var _getOrderedVerseObjec = getOrderedVerseObjectsFromString(verseObjects),
        newVerseObjects = _getOrderedVerseObjec.newVerseObjects;
